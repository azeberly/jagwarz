<?php
    ...
    ),'Title','Description','CorrectAnswer','PointValue','IsComplete','IsOpen','Tags');*/
	...
	*/
	...
	/*Join to get results based on ChallengeId and GameId*/
	//$sJoin = " INNER JOIN GameChallenges ON Challenge.ChallengeId = GameChallenges.ChallengeId";
	//$sJoin = " WHERE Challenge.ChallengeId = 2";
	...
	/*for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if (isset($ChallengeId) && $ChallengeId != '')
		{
			if ($aColumns[$i] == 'ChallengeId')
			{
				...
				$sWhere .= "`".$aColumns[$i]."` = '".mysql_real_escape_string($ChallengeId)."' ";
			}
		}
	}*/
	...
	*/
	...
	$sQuery = "
		SELECT Challenge.ChallengeId, Title, Description, CorrectAnswer, PointValue, IsComplete, IsOpen, Tags, RowPosition 
		FROM Challenge, GameChallenges
		WHERE Challenge.ChallengeId = GameChallenges.ChallengeId
		AND GameChallenges.GameId = " . $GameId . "		
		$sOrder
		$sLimit		
		";
	$rResult = mysql_query( $sQuery, $gaSql['link'] ) or fatal_error( 'MySQL Error: ' . mysql_errno() );
