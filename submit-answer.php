...
{
mysql_select_db($db, $conn);
$sql = "select Title as GameTitle from Game where GameId = " . escapeString($_GET["GameId"]);
$result=mysql_query($sql);
while($row = mysql_fetch_array($result))
	{
...
if (count($UserChallengeArray) > 0)//error: the code here cannot find the proper GameId or ChallengeId, causing answered challenges to be able to answer again
{
	echo "<div class=\"alert alert-danger\">Oops! You have already answered this challenge!</div>";
}
else
{
	$timestamp = time();
	...
	//$UserChallengeSubmission->GameChallengeId = $GCID; //GameChallengeId? GCID?
	$IsCorrect = 0;
	$PointValue = 0;
	...
	if ($UserChallengeSubmission->UserChallengeSubmissionId > 0)
	{
		...
		//$Progress->GameChallengeId = $GameChallengeId;
